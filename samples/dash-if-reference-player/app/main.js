   ...
   // <script src="node_modules/@tensorflow/tfjs/dist/tf.min.js"></script>
    ...
    $scope.chartState = {
        audio: {
            ...
            avgFrameQuality: { data: [], selected: false, color: '#32090a', label: 'Avg Frame Quality' }
        },
        video: {
    ...
    //$scope.video = 'D:/VSCode Projects/dash.js/samples/stream.mpd';
    ...
    //
    ////////////////////////////////////////
    $scope.initSession = async function () {
        $scope.NDGmodel = await tf.loadLayersModel('./converted_tjfs_model/model.json');
    ...
    function runModel(imageData,frameNumber) {
        // Convert imageData to a tensor
        
        const tensor = tf.browser.fromPixels(imageData).toFloat();
    
        // Resize the image to the required size (299x299)
        const resizedTensor = tf.image.resizeBilinear(tensor, [299, 299]);
    
        // Expand dimensions to match the model input shape
        const expandedTensor = resizedTensor.expandDims();
    
        // Preprocess the input tensor
        const preprocessedTensor = expandedTensor.div(tf.scalar(255)).sub(tf.scalar(0.5)).mul(tf.scalar(2));
        
        // Make predictions using the model
        const predictions = $scope.NDGmodel.predict(preprocessedTensor);
    
        // Calculate average prediction
        const avgPred = predictions.mean().dataSync()[0];
    
        // Output or store the result as needed
        console.log('Average Prediction:', avgPred, 'for frame number',frameNumber);
    ...
    $scope.framevaltimer=0;

    function updateMetrics(type) {
        
        
        //console.log('Updating metrics at',getTimeForPlot());
        $scope.framevaltimer+=1;
        ...
        if (dashMetrics && $scope.currentStreamInfo) {
            ...
            if ($scope.chartCount % 1 === 0) {
                ...
                if(type=='video'&&(!($scope.player.isPaused()))&&(($scope.framevaltimer)%7==0))
                {
                    var AvgFrameQuality=processFrame();
                    $scope[type + 'AvgFrameQuality'] = AvgFrameQuality;
                    $scope.plotPoint('avgFrameQuality', type, AvgFrameQuality, time);
    ...
    function processFrame() {
        ...
        var imageElement = document.createElement('mydnamicimg');
        var downloadLink = document.getElementById('downloadLink');

        var currentTime = $scope.video.currentTime;
        var frameRate = 30;
        var frameNumber = Math.floor(currentTime * frameRate);
    
        canvas.width = $scope.video.videoWidth;
        canvas.height = $scope.video.videoHeight;
    
        canvasContext.drawImage($scope.video, 0, 0, canvas.width, canvas.height);
        ...
        var val=runModel(imageData, frameNumber);
        return val;
    ...
    function setLatencyAttributes() {
        ...
        $scope.defaultStableBufferDelay= 600;
        $scope.defaultBufferTimeAtTopQuality = 480;
        $scope.defaultBufferTimeAtTopQualityLongForm = 480;
